#!/usr/bin/env bash
# Startet LiteLLM (Docker) und das Key-Portal lokal.
...
(cd "$PORTAL_DIR" && LC_ALL= LANG= uv run portal.py) &
PORTAL_PID=$!
echo "Warte auf Key-Portal (Port 8080)..."
for i in $(seq 1 30); do
  if nc -z localhost 8080 &>/dev/null; then
    echo "Key-Portal bereit."
    break
  fi
  if [ "$i" -eq 30 ]; then
    echo "WARNUNG: Key-Portal antwortet nicht nach 60s, fahre trotzdem fort." >&2
  fi
  sleep 2
done
...
echo ""
echo "════════════════════════════════════════════════════"
echo "  LiteLLM-URL:  http://localhost:4000"
